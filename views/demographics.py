...
common.hero("Population & demographics",
            "Population structure and how communities compare across the region.")
...
with left:
    ...
    st.plotly_chart(charts.population_pyramid(row), width="stretch")
    st.caption("Age/sex distribution is modelled from population and median "
               "age (single-year tables not in MVP).")
with right:
    ...
    fig = px.scatter(
        ...
        labels={"population": "Population", "pop_growth_pct": "Annual growth (%)",
                "median_age": "Median age"},
    )
    fig.add_hline(y=suburbs["pop_growth_pct"].mean(), line_dash="dot",
                  line_color=COLORS["muted"], annotation_text="region avg")
...
bar = px.bar(inc, x="median_household_income", y="suburb", orientation="h",
             labels={"median_household_income": "Income ($)", "suburb": ""})
