...
def main() -> None:
    common.page_setup("Compare", icon="⚖️")
    common.hero("Compare communities",
                "Put two Wellington communities head-to-head.")

    names = data_loader.suburb_names()
    c1, c2 = st.columns(2)
    a_name = c1.selectbox("Community A", names,
                          index=names.index("Petone") if "Petone" in names else 0)
    default_b = "Johnsonville" if "Johnsonville" in names else names[1]
