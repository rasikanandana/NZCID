"""Hazard analysis module: live GeoNet earthquakes + exposure indices."""
...
def _quakes(mmi: int):
    df = geonet.fetch_recent_quakes(mmi=mmi)
    return geonet.filter_wellington(df)


def main() -> None:
    common.page_setup("Hazards", icon="⚠️")
    common.hero("Hazard analysis",
                "Live earthquakes from GeoNet plus flood, tsunami and "
                "landslide exposure indices.")
    ...
    st.caption("Flood, tsunami and landslide indices are indicative MVP "
               "composites. For authoritative hazard information consult "
               "Greater Wellington Regional Council and NEMA.")
